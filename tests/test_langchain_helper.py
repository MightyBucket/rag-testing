...
def helper_module():
    ...
    # Use mock classes for the LLM and embedding to avoid real API calls during unit tests
...
def test_load_csv_documents_populates_source_metadata(tmp_path, helper_module):
    ...
    # Check the document comes from the expected source
...
def test_get_retrieved_documents_requires_existing_faiss_index(helper_module):
    """Retrieval should give a clear error when the FAISS knowledge base has not been created."""
...
def test_get_retrieved_documents_returns_similarity_docs(helper_module):
    ...
    # Make simple vector store that only has a single document in it
    ...
    # Do a simple search for a restaurant
    ...
    # Make sure that single document is returned
...
def test_create_combined_vector_db_builds_and_saves_mock_faiss(helper_module):
    ...
    # Make another fake dataset from tiktok and reddit
    fake_docs = [
    ...
    # Make a fake vector embedding for these documents
...
def test_get_qachain_requires_existing_faiss_index(helper_module):
    """Make sure an error is returned if trying to build the RAG but the vector store is missing."""
    with patch.object(helper_module.os.path, "exists", return_value=False):
        with pytest.raises(FileNotFoundError, match="FAISS index does not exist"):
